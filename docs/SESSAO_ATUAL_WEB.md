...
*Última atualização: 4 de julho de 2026 — Web: empty state honesto em Transações. Só o ramo `transactions.length === 0` mudou de texto (título "Nenhuma transação registrada em [mês] [ano]" + descrição direcionando as parcelas ao Dashboard/Resumo); ramo de filtros intacto; contagem do topo rotulada "transações registradas" (cálculo intocado). Sem redesenho, sem tooltip. `npm run build` verde.*
