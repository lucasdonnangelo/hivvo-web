...
*Última atualização: 1 de julho de 2026 — Web T-28: base URL apontada para `/api/v1` (fallback de [api.ts](../src/services/api.ts) → `http://localhost:8000/api/v1`, `.env.example` documentado dev/produção; nenhum path de service tocado — o `/api/v1` vem da `baseURL`; frontend não chama `/health`); `npm run build` verde.*
